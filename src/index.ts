...
${dirty ? '⚠️ **Warning:** There were uncommitted changes when this was pushed.\n' : ''}
    ...
    try {
      ...
      for (const folder of pulledFolders) {
        ...
        if (syncData) {
          ...
          if (!commitExists) {
            console.log(chalk.yellow(`\n⚠️  Code changes exist on commit ${syncData.commitHash}`));
          ...
          if (syncData.dirty) {
            console.log(chalk.yellow(`\n⚠️  There were uncommitted changes when this was pushed.`));
          }
        }
      }
    } finally {
        ...
        if (localFolders.length > 0) {
          ...
          for (const folder of localFolders) {
            ...
            if (syncData) {
              ...
              console.log(chalk.gray(`  └─ JIRA synced at: ${syncData.commitHash} (${syncData.branch})`));

              if (ahead === -1) {
                // Commit not in current branch
                const currentBranch = await getCurrentBranch();
                console.log(chalk.yellow(`  └─ ⚠️  You're on '${currentBranch}' but spec was pushed from '${syncData.branch}'`));
              } else if (ahead === 0) {
                // Up to date
                console.log(chalk.green(`  └─ ✓ Up to date with HEAD (${currentCommit})`));
              } else {
                // Ahead
                console.log(chalk.gray(`  └─ Your HEAD: ${currentCommit} (${ahead} commit${ahead > 1 ? 's' : ''} ahead)`));
                console.log(chalk.cyan(`\n💡 You have newer commits. Run 'openspec-sync push' to update JIRA.`));
              }
            } else {
              console.log(chalk.gray(`  └─ No SYNC.md found (not yet pushed with tracking)`));
            }
          }
        }
      }
